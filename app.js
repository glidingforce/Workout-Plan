...
const backBtn = (onClick) => html`
    <button onClick=${onClick} style=${{
        background:'transparent', border:'none', color:C.gray,
        fontSize:'22px', padding:'4px 8px 4px 0', lineHeight:1,
...
const closeBtn = (onClick) => html`
    <button onClick=${onClick} style=${{
        background: C.secondary, border:'none', color:C.gray,
...
function downloadTemplate(workouts) {
    ...
    const instrData = [
        ...
        ['HOW TO USE:'],
        ['1. Edit the "Workouts" sheet with your exercises'],
        ['2. Do NOT change the column headers (row 1)'],
        ['3. GroupID: short unique ID, no spaces (e.g. push_a) — keep existing ones to preserve images'],
        ['4. GroupIcon: emoji for the group'],
        ['5. Group: group display name'],
        ['6. Exercise: exercise name'],
        ['7. Sets: number'],['8. Reps: text (e.g. 8-10 or 30 sec)'],
        ['9. Weight_kg: number (0 = bodyweight)'],
        ['10. Rest_sec: rest seconds after each set'],
        ['11. Notes: optional form tips'],[''],
        ['Save as .xlsx and upload in app → Import / Export → Upload Excel'],
    ];
    ...
    workouts.forEach(w => {
        w.exercises.forEach((ex,i) => {
            rows.push([w.id,w.icon,w.name,i+1,ex.name,ex.sets,ex.reps,ex.weight,ex.rest,ex.notes||'']);
        });
    });
...
function parseImportedXlsx(arrayBuffer) {
    ...
    rows.forEach((row,i) => {
        ...
        if (!exName) return;
        if (!groupMap[gid]) { groupMap[gid]={id:gid,icon:gicon,name:gname,exercises:[]}; groupOrder.push(gid); }
        const newId = groupMap[gid].exercises.length+1;
        groupMap[gid].exercises.push({
            id:newId, name:exName,
    ...
    return html`
        <div class="modal-overlay" onClick=${onCancel}>
            <div class="modal-box" onClick=${e=>e.stopPropagation()}>
                ...
                <input type="text" value=${typed} onInput=${e=>setTyped(e.target.value)}
                    placeholder="Type DELETE"
                    style=${{
                        width:'100%', background:C.secondary,
                        border:`1px solid ${ok?C.red:C.secondary}`,
                ...
                <div style=${{display:'flex',gap:'10px'}}>
                    ...
                    <button onClick=${ok?onConfirm:undefined} style=${{
                        flex:1, background:ok?C.red:C.secondary,
                        color:ok?C.white:C.gray, border:'none', padding:'11px',
...
function HomeScreen({ workouts, history, onStart, onEdit, onAddGroup, onImport }) {
    function handleClose() {
        // Works when added to iPhone home screen as PWA
        // Falls back gracefully in browser
        if (window.navigator.standalone) {
            // PWA mode – navigate to blank to "close"
    ...
    return html`
        <div class="screen">
            <!-- Top bar: logo + title + close -->
            <div style=${{display:'flex',alignItems:'center',gap:'10px',marginBottom:'6px'}}>
                <img src="images/icon.png" style=${{width:'40px',height:'40px',borderRadius:'10px',flexShrink:0}} />
                <h1 style=${{fontSize:'26px',fontWeight:'700',color:C.yellow,flex:1}}>NextUp</h1>
...
function EditScreen({ workout, onSave, onBack, onDeleteGroup }) {
    ...
    function updateEx(idx,field,val) {
        setLocal(l=>{const n=JSON.parse(JSON.stringify(l));n.exercises[idx][field]=val;return n;});
    }
    function addEx() {
        setLocal(l=>{
    ...
    function deleteEx(idx) {
        setLocal(l=>{const n=JSON.parse(JSON.stringify(l));n.exercises.splice(idx,1);return n;});
    ...
    return html`
        <div class="screen">
            ...
            <div style=${{display:'flex',alignItems:'center',gap:'8px',marginBottom:'14px'}}>
                ${backBtn(onBack)}
                <h1 style=${{fontSize:'19px',fontWeight:'700',margin:0,flex:1}}>${local.icon} ${local.name}</h1>
            </div>
    ...
    return html`
        <div class="screen">
            ...
            <div style=${{display:'flex',alignItems:'center',gap:'8px',marginBottom:'24px'}}>
                ${backBtn(onBack)}
                <h2 style=${{fontSize:'20px',fontWeight:'700',margin:0,color:C.yellow}}>New Workout Group</h2>
            </div>
...
function ImportScreen({ workouts, onImported, onBack }) {
    ...
    async function handleFile(file) {
        ...
        } catch(e) { setStatus('error'); setMsg(e.message||'Failed to parse file'); }
    }
    ...
    return html`
        <div class="screen">
            ...
            <div style=${{background:C.card,border:`1px solid ${C.secondary}`,borderRadius:'12px',padding:'16px',marginBottom:'16px'}}>
            ...
            <div style=${{background:C.card,border:`1px solid ${C.secondary}`,borderRadius:'12px',padding:'16px',marginBottom:'16px'}}>
                ...
                <p style=${{fontSize:'12px',color:C.gray,marginBottom:'12px',lineHeight:'1.5'}}>
                    ...
                    <strong style=${{color:C.orange}}>⚠ This replaces all groups (images kept if GroupID unchanged).</strong>
                </p>
                <div
                ...
                ${status==='success' && html`
                    <div style=${{marginTop:'12px',background:'#0d1f0d',border:'1px solid #2d6a2d',borderRadius:'8px',padding:'10px 12px',color:'#66bb66',fontSize:'13px'}}>✅ ${msg}</div>`}
                ${status==='error' && html`
                    <div style=${{marginTop:'12px',background:'#1f0d0d',border:`1px solid ${C.red}`,borderRadius:'8px',padding:'10px 12px',color:C.red,fontSize:'13px'}}>❌ ${msg}</div>`}
            </div>
...
function WorkoutScreen({ workout, onFinish, onExit }) {
    ...
    useEffect(()=>{ setCachedImage(LS.img(workout.id,ex.id)); }, [exIdx]);

    useEffect(()=>{
        if (!restActive||restTime===null||restTime<=0) { if(restTime===0)setRestActive(false); return; }
        const t=setTimeout(()=>setRestTime(v=>v-1),1000);
        return ()=>clearTimeout(t);
    },[restActive,restTime]);

    function goNextExercise() { setExIdx(i=>i+1); setSetNum(1); setRestActive(false); setRestTime(null); }

    function handleSetDone() {
        if (!isLastSet) {
            setSetNum(s=>s+1);
            if (ex.rest>0) { setRestTime(ex.rest); setTotalRest(ex.rest); setRestActive(true); }
        } else if (!isLastExercise) { goNextExercise(); }
        else { onFinish(); }
    }

    function handleSkip() { setShowSkip(false); if(!isLastExercise)goNextExercise(); else onFinish(); }

    const restPct = totalRest ? (restTime/totalRest)*100 : 0;
    const btnLabel = (isLastSet&&isLastExercise) ? '🏁 Complete Workout'
        : isLastSet ? '✓ Complete Set  →  Next'
        : setNum===1 ? '▶ Start'
        : '✓ Complete Set';
    ...
    return html`
        <div class="screen" style=${{display:'flex',flexDirection:'column',minHeight:'100vh'}}>
            ...
            <!-- Header: back arrow + workout info + progress dots -->
...
function App() {
    ...
    function go(sc,id=null){setActiveId(id);setScreen(sc);}

    function handleSaveEdit(updated) { setWorkouts(ws=>ws.map(w=>w.id===updated.id?updated:w)); go('home'); }
    function handleDeleteGroup(id)   { setWorkouts(ws=>ws.filter(w=>w.id!==id)); go('home'); }
    ...
    if (screen==='home')     return html`<${HomeScreen} workouts=${workouts} history=${history} onStart=${id=>go('workout',id)} onEdit=${id=>go('edit',id)} onAddGroup=${()=>go('addgroup')} onImport=${()=>go('import')} />`;
    if (screen==='addgroup') return html`<${AddGroupScreen} onConfirm=${handleAddGroup} onBack=${()=>go('home')} />`;
    if (screen==='import')   return html`<${ImportScreen} workouts=${workouts} onImported=${setWorkouts} onBack=${()=>go('home')} />`;
    if (screen==='edit'&&activeWorkout) return html`<${EditScreen} workout=${activeWorkout} onSave=${handleSaveEdit} onBack=${()=>go('home')} onDeleteGroup=${()=>handleDeleteGroup(activeId)} />`;
    if (screen==='workout'&&activeWorkout) return html`<${WorkoutScreen} workout=${activeWorkout} onFinish=${handleFinish} onExit=${()=>go('home')} />`;
    if (screen==='finish'&&activeWorkout) return html`<${FinishScreen} workout=${activeWorkout} onHome=${()=>go('home')} />`;
    return null;
}
